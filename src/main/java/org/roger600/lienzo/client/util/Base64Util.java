...
public class Base64Util {
     ...
     * @param data the data to encode. It is assumed that none of the elements are
     * null.
     * @param off The offset into the data array to start encoding from
     * @param length The number of bytes to encode, starting from {@code off}
    ...
    public static byte[] decode(final String data) {
        ...
        while (i < finish) {
            out[outpos++] = (byte) ((decodingTable[data.charAt(i = nextI(data,
                                                                         i,
                                                                         finish))] << 2) | ((b2 = decodingTable[data.charAt(i = nextI(data,
                                                                                                                                      ++i,
                                                                                                                                      finish))]) >> 4));
            out[outpos++] = (byte) ((b2 << 4) | ((b3 = decodingTable[data.charAt(i = nextI(data,
                                                                                           ++i,
                                                                                           finish))]) >> 2));
            out[outpos++] = (byte) ((b3 << 6) | decodingTable[data.charAt(i = nextI(data,
                                                                                    ++i,
                                                                                    finish))]);
    ...
    private static int nextI(final String data,
                             int i,
                             final int finish) {
